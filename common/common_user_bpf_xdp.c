...
struct bpf_object *load_bpf_and_xdp_attach(struct config *cfg)
{
	...
	// Olaf code moment:
	// Hier probeer ik van de net ingeladen bpf_obj de ELF sections op te halen,
	// om vervolgens een entry in .rodata (RULE_0_ENABLED). De data hiervoor
	// haal ik uit de config (cnf->link_test)

	//btf *btf = bpf_object__btf(bpf_obj);
...
static const char *xdp_action_names[PT_MAX] = {
